...
async function fetchMarketData(symbol: string): Promise<{ price: number; prevClose: number } | null> {
  ...
  try {
    ...
    return { price: parseFloat(quote["05. price"]), prevClose: parseFloat(quote["08. previous close"]) };
  } catch {
    return null;
  }
}
...
const checkAlert = (value: number, prev: number, trend: number, type: string) => {
  ...
  if (type === "efficiency" && value < 30) return { triggered: true, message: "Efficiency below 30% threshold", severity: "high" as const };
...
export async function GET() {
  ...
  try {
    ...
    const revTrend = previousRevenue > 0 ? ((currentRevenue - previousRevenue) / previousRevenue) * 100 : 0;
    ...
    const currentEff = currentM ? (currentM.profit / Math.max(currentM.revenue, 1)) * 100 : 0;
    const prevEff = prevM ? (prevM.profit / Math.max(prevM.revenue, 1)) * 100 : 0;
    const effTrend = prevEff > 0 ? ((currentEff - prevEff) / prevEff) * 100 : 0;
    const effInsight = await getAIInsight("Operational Efficiency (Est.)", currentEff, effTrend);
    ...
    const marketData: RealTimeData["marketTrends"] = marketLive
      ? (() => {
          ...
          return {
            ...
            alert: checkAlert(marketLive.price, marketLive.prevClose, mktTrend, "market"),
            aiInsight: "",
      ...
      : {
          ...
          trendPercent: 0,
          alert: null,
          aiInsight: "Market data unavailable — check ALPHA_VANTAGE_KEY or rate limit.",
          lastUpdated: new Date().toISOString(),
    ...
    if (marketLive) {
      marketData.aiInsight = await getAIInsight("S&P 500 (SPY)", marketData.current, marketData.trendPercent);
    }
